...
def view(request, id, slug=None):
    ...
    if not backgrounds:
        ...
        # This has to happen as the last step,
        # so it is rendered behind the muscles.
...
class ExercisesEditAddView(WgerFormMixin):
    ...
    def get_form_class(self):
        # Define the exercise form here because only
        # at this point during the request
        # have we access to the currently used language.
...
class ExerciseCorrectView(ExercisesEditAddView, LoginRequiredMixin,
    ...
    messages = _(
        'Thank you. Once the changes are '
        'reviewed the exercise will be updated.')
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(ExerciseCorrectView, self).dispatch(request, *args,
                                                         **kwargs)
    ...
    def form_valid(self, form):
        """
        If the form is valid send email notifications
